...
# ca-certificates will fail but git is usable for our purpose (setup version info)
# also set push to default in case clone has been done with a recent git version which has simple mode as default
devimage/bin/ckains -S rootfs-x86_64 -- sh -c '(apt-get install -y git || true) && git config --global push.default matching'
...
# ca-certificates will fail but git is usable for our purpose (setup version info)
# also set push to default in case clone has been done with a recent git version which has simple mode as default
devimage/bin/ckains --32 -S rootfs-i686 -- sh -c '(apt-get install -y git || true) && git config --global push.default matching'
